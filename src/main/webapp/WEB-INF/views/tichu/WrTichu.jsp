...
<h2 class="page-section-heading text-center text-uppercase text-secondary mb-0">티츄카운터</h2>
 ...
 <tbody>
  <c:forEach var="l" items="${tichulist}">
                  <tr onclick="location.href='/tichudetail/${l.TB_index}'">
<!-- 						<tr> -->
<%--                       <td>${l.tb_board_bno}</td> --%>
<td>${l.TB_member_Name }</td>
<td>${l.TB_member_Name }님의 글</td>
                      <td>
    ...
    <c:if test="${pageMaker.prev }">
        ...
        <a href='<c:url value="/tichu?page=${pageMaker.startPage-1 }"/>'><i class="fa fa-chevron-left"></i></a>
    
    </c:if>
    <c:forEach begin="${pageMaker.startPage }" end="${pageMaker.endPage }" var="idx">
        <a href='<c:url value="/tichu?page=${idx }"/>'><i class="fa">${idx } </i> </a>
...
<div  align="right">
        <c:if test="${not empty memberinfo.TB_member_ID }">
         <button class="btn btn-primary btn-sm" style="float:right;" onclick="location.href='/tichu_insertPage'">글쓰기</button>
         </c:if>
         </div>
      </div>
    </div>
