...
<html>
 ...
 <tr onclick="location.href='/noticeboarddetail/${notice.tb_board_bno}'" bgcolor="gray">
  ...
  <c:forEach var="l" items="${list}">
                  <tr onclick="location.href='/freeboarddetail/${l.tb_board_bno}'">
    ...
    <c:if test="${pageMaker.prev }">
        ...
        <a href='<c:url value="/LotteFreeBrd?page=${pageMaker.startPage-1 }"/>'><i class="fa fa-chevron-left"></i></a>
    
    </c:if>
    <c:forEach begin="${pageMaker.startPage }" end="${pageMaker.endPage }" var="idx">
        <a href='<c:url value="/LotteFreeBrd?page=${idx }"/>'><i class="fa">${idx } </i> </a>
     ...
     <div  align="right" style="margin-top: 15px;">
        <c:if test="${not empty memberinfo.TB_member_ID }">
         <a class="btn btn-outline-secondary" style="float:right;" onclick="location.href='/free_board_insert'">글쓰기</a>
         </c:if>
         </div>
    </div>
    </div>
