...
<html>
 ...
 <thead>
  ...
  <c:forEach var="l" items="${list}">
                  <tr onclick="location.href='/noticeboarddetail/${l.tb_board_bno}'">
    ...
    <c:if test="${pageMaker.prev }">
        ...
        <a href='<c:url value="/LotteNoticeBrd?page=${pageMaker.startPage-1 }"/>'><i class="fa fa-chevron-left"></i></a>
    
    </c:if>
    <c:forEach begin="${pageMaker.startPage }" end="${pageMaker.endPage }" var="idx">
        <a href='<c:url value="/LotteNoticeBrd?page=${idx }"/>'><i class="fa">${idx } </i> </a>
     ...
     <div  align="right" style="margin-top: 15px;">
        <c:if test="${not empty memberinfo.TB_member_ID }">
         <a class="btn btn-outline-secondary" style="float:right;" onclick="location.href='/NoticeBoardInsert'">글쓰기</a>
         </c:if>
         </div>
    </div>
    </div>
